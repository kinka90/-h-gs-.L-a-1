...
(function(){
  ...
  async function startQuiz(){
    ...
    try {
      ...
      // show intro again after 2s so user can retry
  ...
  async function fetchQuestionsFromAI(topic, count, lang){
    ...
    try {
      ...
      clearTimeout(id);
      throw new Error('Gagal terhubung ke AI (timeout atau jaringan).');
    }
    clearTimeout(id);
  ...
  async function reviewAnswerWithAI(questionObj, userChoice, lang){
    ...
    try {
      ...
      clearTimeout(id);
      throw new Error('Gagal menghubungi AI (timeout/jaringan).');
    }
    clearTimeout(id);
  ...
  async function finishAndScore(){
    ...
    try {
      ...
      const resp = await fetch(API_PROXY, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ model: MODEL, messages:[sys,user], temperature:0.4 }), signal: (new AbortController()).signal });
      if(resp.ok){
        const j = await resp.json();
